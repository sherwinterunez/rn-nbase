...
export default class ListItemNB extends NativeBaseComponent {
    ...
    getInitialStyle() {
        ...
        return {
            ...
            itemText: {
                fontSize: this.getTheme().listItemTextSize,
                marginLeft: (this.props.iconLeft) ? this.getTheme().listItemTextMarginLeft : 0,
                color: this.getContextForegroundColor(),
                alignSelf: ((this.pickerPresent() && this.props.iconLeft) || (this.props.iconLeft && this.badgePresent()) || (this.checkBoxPresent() && this.buttonPresent()) ) ? 'center' : 'center',
                //paddingBottom: (this.props.iconLeft) ? this.getTheme().listItemTextPaddingBottom : 0,
                //borderWidth: 1,
                //borderColor: '#f00',
            },
            dividerItemText: {
    ...
    renderChildren() {
        ...
        if(!Array.isArray(this.props.children) && !this.inlinePresent() && !this.stackedPresent() && !this.insetPresent()) {
            ...
            if (this.props.iconLeft && !this.props.iconRight && !this.pickerPresent()) {
                ...
                //console.log('terunez #13');

                newChildren.push(<View key='listItem0' style={{flexDirection: 'row', alignSelf: 'center', flex: 1 , padding: 0}} >
                    {childrenArray.map((child, i) => {
                        return React.cloneElement(child, {...this.getChildProps(child), key: i});
                    })}
                    </View>);
